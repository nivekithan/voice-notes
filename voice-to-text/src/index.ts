	...
	async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		try {
			...
			if (isCreatingNewNote) {
				const { content, title, transcript } = await createNewNote({ audio, env, systemMessage });
				return Response.json({ transcript, title, content });
			}

			const { text } = await updateNote({ audio, currentText, env, systemMessage });
...
async function createNewNote({ audio, systemMessage, env }: { audio: File; systemMessage: string; env: Env }) {
	const openAi = new OpenAI({ apiKey: env.OPENAI_API_KEY });
