...
export async function createNewNotes({ db, id, content, status, title, transcript, userId }: WithDb<CreateNotes>) {
	const notes = await db.insert(Notes).values({ id, content, status, title, transcript, userId }).returning();
...
export async function getAllNotes({ userId, db }: WithDb<{ userId: string }>) {
	const notes = await db.select().from(Notes).where(eq(Notes.userId, userId)).orderBy(desc(Notes.createdAt));

	return notes;
}

export async function getNotes({ db, noteId, userId }: WithDb<{ noteId: string; userId: string }>) {
